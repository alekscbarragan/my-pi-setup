/** Model-facing schema descriptions for the ben_ask_user question and answer options. */
...
/** Describes the ben_ask_user tool's question shape and dismissible free-form fallback. */
...
/** Adds ben_ask_user's multiple-choice capability to the model's available-tools prompt. */
export const ASK_USER_PROMPT_SNIPPET =
  "Ask the user a multiple-choice question (2-5 options plus a free-form answer)";

/** Guides the model to use ben_ask_user for enumerable answers and one question at a time. */
export const ASK_USER_PROMPT_GUIDELINES = [
  "When asking the user a question whose likely answers can be enumerated, use the ben_ask_user tool instead of asking in plain text.",
  "Ask one question per ben_ask_user call; ask follow-up questions in subsequent calls.",
];

/** Builds the behavioral tool-result message returned to the parent model for a ben_ask_user outcome. */
